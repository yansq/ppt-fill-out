...
export default async function FillInstancePage({
  params,
}: {
  ...
  try {
    ...
    if (error instanceof AuthorizationError)
      redirect(`/login?callbackUrl=/fill-instances/${instanceId}`);
    if (error instanceof ReportTaskError) notFound();
    throw error;
  }
  const actor = await currentActor();
  const pages = instance.editable
    ? await listMyTaskPages(instance.task.id)
    : [];
  const navigation = pages.some((page) => page.id === instance.id)
    ? { currentId: instance.id, pages }
    : null;
  return (
    <WorkspaceShell
    ...
    >
      <main className="page-container fill-page-container">
        ...
        {instance.returnReason ? (
          <p
            className="mb-6 rounded-md border border-primary bg-accent p-4 text-sm"
            role="status"
          >
            退回原因：{instance.returnReason}
          </p>
        ) : null}
        {instance.status === "IN_PROGRESS" ||
        instance.status === "SUBMITTED" ? (
          <FillInEditor initialInstance={instance} navigation={navigation} />
        ) : (
          <div className={`fill-page-grid ${navigation ? "has-page-nav" : ""}`}>
            ...
            <section className="min-w-0 rounded-xl border bg-card p-4 xl:sticky xl:top-24">
              ...
              <PptPreviewImage
                alt={`第 ${instance.slideIndex + 1} 页模板预览`}
                aspectRatio={instance.slideAspectRatio}
                key={instance.previewUrl}
                ...
                <ul className="space-y-2">
                  {instance.placeholders.map((placeholder) => (
                    ...
                    >
                      ...
                      <span>#{placeholder.occurrenceIndex + 1}</span>
                      <p className="mt-1">{placeholder.originalText}</p>
                    </li>
                  ))}
                </ul>
              ) : (
                <p className="text-sm">此页没有动态占位符。</p>
              )}
              {instance.editable &&
              (instance.status === "NOT_STARTED" ||
                instance.status === "RETURNED") ? (
