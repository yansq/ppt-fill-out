...
import { MetricServiceError } from "@/features/metric/metric-service";
import { ReportTaskError } from "@/features/report-task/report-task-service";

export function fillInErrorResponse(error: unknown) {
  ...
  return NextResponse.json({ error: { code: "DATABASE_UNAVAILABLE", message: "填报服务暂时不可用" } }, { status: 503 });
}
