...
export async function getDraftPreview(instanceId: string, expectedVersion: number) {
  const instance = await getFillInstance(instanceId);
  if (instance.version !== expectedVersion) {
    throw new ReportTaskError("VERSION_CONFLICT", "填报草稿已更新，请刷新预览", 409);
  }
  ...
  const bindings = new Map(instance.bindings.map((binding) => [binding.placeholderId, binding]));
  const values = instance.placeholders.map((placeholder) => {
    const binding = bindings.get(placeholder.id);
    const snapshot = binding?.sourceSnapshotJson as { valueText?: unknown } | null;
    return {
      key: placeholder.key,
      occurrenceIndex: placeholder.occurrenceIndex,
      valueText: binding?.sourceType === "MANUAL_TEXT"
        ? binding.manualValue ?? ""
        : typeof snapshot?.valueText === "string" ? snapshot.valueText : ""
    };
  });
