...
export function GenerationPanel({ initial }: { initial: GenerationList }) {
  ...
  const pages = latest
    .filter((file) => file.type === "PNG")
    .sort((a, b) => (a.slideIndex ?? 0) - (b.slideIndex ?? 0));
  const warnings = pptx?.warnings ?? [];
  const canGenerate =
    data.task.status === "COMPLETED" || data.task.status === "EXPORTED";
  ...
  async function generate() {
    ...
    try {
      const response = await fetch(
        `/api/report-tasks/${data.task.id}/generate`,
        {
          method: "POST",
      ...
      const result = (await response.json()) as GenerationList & {
        error?: {
          message: string;
          details?: { key: string; slideIndex: number }[];
        };
      };
      if (!response.ok) {
        const missing = result.error?.details
          ?.map((item) => `第 ${item.slideIndex + 1} 页 {{${item.key}}}`)
          .join("、");
        throw new Error(
          `${result.error?.message ?? "生成失败"}${missing ? `：${missing}` : ""}`,
        );
      }
      setData(result);
  ...
  async function exportFile(generatedFileId: string) {
    ...
    try {
      const response = await fetch(
        `/api/report-tasks/${data.task.id}/exports`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  return (
    <section className="mt-10 rounded-lg border bg-card p-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold">生成与导出报告</h2>
          <p className="mt-1 text-sm">
            使用已确认的最终内容生成，请逐页检查后下载。
          </p>
        </div>
        <Button
      ...
      {!canGenerate ? (
        <p className="mt-4 text-sm">完成审核后才能生成报告。</p>
      ) : null}
      {latest.length > 0 ? (
        <div className="mt-6 space-y-5">
          ...
          {warnings.length ? (
            <div
              className="rounded-md border border-primary bg-accent p-3 text-sm"
              role="alert"
            >
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            {pages.map((page) => (
              <article className="rounded-md border p-3" key={page.id}>
                <h3 className="mb-2 font-medium">
                  第 {(page.slideIndex ?? 0) + 1} 页
                </h3>
                {page.previewUrl ? (
                  <Image
                    alt={`第 ${(page.slideIndex ?? 0) + 1} 页真实预览`}
