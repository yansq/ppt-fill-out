...
- 指标查询使用仅到月份的日历控件；没有任何已配置指标值的月份置灰且不可选择。创建报告任务的报告月份仍可选择无指标月份，以支持纯人工填报。
- 将占位符绑定到指标或录入人工文本。
...
- 选择有数据的月份后自动加载指标，以表格展示名称、编码、数据源、当月值、版本和更新时间。
- 支持按指标名称、编码或数据源搜索，并在服务端分页；当月无值的已配置指标保留在目录中并标明状态。
- 展开单行查看历史、同步记录，以及在有权限时填写修改值和原因。
