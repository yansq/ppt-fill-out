...
交付说明：按用户要求，本次界面变更暂不更新既有内网镜像归档；`dist/intranet-2026-09-23/images.tar.gz` 仍对应前一版应用代码，部署包校验清单保持不变。

### 2026-09-23 — 指标管理页切换月份自动加载

- [x] 指标管理页在所选月份有数据时自动查询，切换月份直接显示该月指标；移除“查询指标”按钮并更新页面提示。
- [x] 切换月份或离开页面时取消旧请求，失败时显示原因和重试按钮。

验证：`pnpm lint`、`pnpm typecheck`、Web 27 文件/81 项测试和 Next.js build 通过；新增交互测试覆盖初次加载、月份切换和失败重试。按用户要求，既有内网镜像归档暂不更新。

### 2026-09-23 — 指标管理表格、分页与搜索

- [x] 指标管理页改为表格，展示已配置指标的名称、编码、数据源、当月值、版本和更新时间；按行展开修改、历史与同步操作，当月无值的指标明确标记。
- [x] 新增仅 Collector 可访问的指标目录接口，按名称、编码或数据源名称搜索，固定每页 20 条；先分页平台指标定义，再按数据源分组批量读取本页当月值，避免页面一次加载全部指标和逐项查询。
- [x] 搜索输入延迟 300 毫秒触发，搜索与月份切换返回第一页；翻页、搜索、切换月份取消旧请求，并保留加载失败重试。

验证：`pnpm lint`、`pnpm typecheck`、Web 28 文件/83 项测试及 Next.js build 通过；本地真实 MySQL 上验证 Prisma 的搜索、关联排序和分页查询，交互测试覆盖表格、翻页、搜索、月份切换与失败重试。按用户要求，既有内网镜像归档仍暂不更新。

### 2026-09-23 — 填报与审核工作区首屏布局

- [x] 报告任务各状态页将顶部提示、步骤与四栏统计压缩为任务标题、操作入口和一行进度摘要，移除审核工作区上方多余间距，让 PPT 预览更早出现在首屏。
- [x] 填报人单页各状态同步缩小标题、说明和页面上边距；退回原因仍单独显示。

验证：`pnpm lint`、`pnpm typecheck`、Web 28 文件/83 项测试及 Next.js build 通过。浏览器访问本地报告任务时被引导至登录页，尚未完成登录后的实际视觉回归。按用户此前要求，既有内网镜像归档仍暂不更新。

### 2026-09-23 — 收集人可承担填报任务

- [x] 活跃收集人的有效权限包含填报人权限，可在“我的填报”查看并处理分配给自己的页面；填报实例仍只允许实际被分配人编辑和提交。
- [x] 页面分配的候选人员和保存校验均接受活跃收集人；角色继承规则与实例权限边界记录于 ADR-0018。

验证：`pnpm lint`、`pnpm typecheck`、Web 29 文件/84 项测试、Next.js build 与 Prisma validate 通过。Prisma validate 使用仅供模式校验的临时 `DATABASE_URL`，未连接数据库。按用户此前要求，既有内网镜像归档仍暂不更新。

### 2026-09-23 — 待分配任务隐藏审核区域

- [x] 报告任务处于 `DRAFT` 时仅展示页面分配，隐藏“页面状态与审核”，同时跳过未使用的审核数据查询；进入填报阶段后继续显示逐页填报与审核工作区。

验证：`pnpm lint`、`pnpm typecheck` 和 Next.js build 通过。此次仅调整服务端状态分支与数据加载，没有新增自动化测试；登录后的页面视觉效果尚未实测。按用户此前要求，既有内网镜像归档仍暂不更新。

### 2026-09-24 — 统一下拉选择组件

- [x] 新增共用 `Select`，支持统一样式、鼠标选择、方向键与 Enter 选择、Escape 关闭及表单值提交。
- [x] 新建任务模板、填报来源、填报指标和审核指标全部改用共用组件；页面代码中不再使用原生 `<select>`。

验证：`pnpm lint`、`pnpm typecheck`、Web 30 文件/86 项测试及 Next.js build 通过。月份选择器与人员多选器沿用各自的非原生交互。登录后的页面视觉效果尚未实测；既有内网镜像归档未更新。

### 2026-09-24 — 用户自行修改密码

- [x] 工作区右上角增加“修改密码”入口，收集人和填报人均可进入账号设置页，输入当前密码及两次新密码。
- [x] 新增仅针对当前登录用户的改密接口：验证旧密码、按内网要求校验新密码至少 6 位及确认值，使用随机盐 scrypt 重新哈希；错误旧密码沿用五次失败锁定策略，并发改密以旧哈希作为更新条件。
- [x] 改密成功写入不含密码与哈希的操作日志；新增服务、表单和哈希兼容性测试。
- [x] 改密成功后自动退出当前会话并跳转登录页，提示使用新密码重新登录；退出失败时留在当前页面提示手动退出。

验证：`pnpm lint`、`pnpm typecheck`、Web 32 文件/96 项测试、Next.js build 及 Prisma validate 均通过。Prisma validate 使用仅供模式校验的临时 `DATABASE_URL`，未连接数据库。登录后的页面视觉效果尚未实测，既有内网镜像归档未更新。

### 2026-09-24 — 模板上传文件选择框中文化

- [x] 模板上传隐藏原生文件输入外观，改用统一的中文“选择文件”按钮和文件名显示；未选择文件时显示“未选择文件”，长文件名截断并保留悬停完整名称。
- [x] 保留系统文件选择器及 PPTX 类型限制，上传成功后清空文件名；未选文件提交时给出中文提示。

验证：`pnpm lint`、`pnpm typecheck`、Web 33 文件/98 项测试及 Next.js build 通过；交互测试覆盖选中文件、FormData 上传、成功清空和空文件提示。登录后的页面视觉效果尚未实测；既有内网镜像归档未更新。

### 2026-09-24 — 填报人自行注册与按工号分配

- [x] 开放六位工号、姓名和至少六位密码的公开注册；只授予填报人角色，已注册、停用和收集人账号不能经此入口覆盖。
- [x] 收集人可输入六位工号暂存待分配人员；保存分配时在同一事务中按需创建姓名为空、无登录凭据的填报人账号及关联的分配和填报实例，不预导入员工名册。
- [x] 员工之后注册时在原 User ID 上补全姓名及凭据，既有分配保持关联；未注册员工在分配和审核界面显示工号，不展示内部占位用户名。
- [x] 注册成功后直接使用刚提交的工号与密码建立 Auth.js 会话并进入工作台；自动登录失败时提示账号已创建，并提供登录入口，不重复提交注册。
- [x] ADR-0019、接口契约和领域模型同步记录规则与未校验工号归属的风险。

验证：`pnpm lint`、`pnpm typecheck`、Web 36 文件/109 项测试、Next.js build 及 Prisma validate 均通过。注册表单测试覆盖自动登录进入工作台和自动登录失败提示。Prisma validate 使用仅供模式校验的临时 `DATABASE_URL`，未连接数据库；未对真实账号执行注册或分配端到端测试。Java 服务与数据库模式未变，未重复运行 Java 构建。既有内网镜像归档未更新。
...
- 公开注册暂不验证工号归属；知道他人工号的人可抢先注册并查看该工号已分配的任务，正式开放前需接受此风险或接入企业身份认证。
