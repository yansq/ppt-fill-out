...
| P4 指标与填报 | 进行中 | 85% | 独立示例指标库、查询/修改/历史、绑定、人工草稿、近似 Fast Preview 与提交快照已通过真实账号冒烟；真实企业映射及去字背景未完成 |
...
- P4 收尾：按企业真实指标库的表结构实现受限映射；生成真正去字的静态预览背景，并验证容器内数据源连通性与跨库失败恢复。

## 下一步

1. 在企业目标内网取得真实指标库表结构、权限与连接参数，将示例固定映射扩展为受控可配置映射；禁止原始 SQL 输入。
2. 由 PPT Service 生成动态占位符去字背景并回归混合字体/表格的 Fast Preview；继续以 P6 Real Preview 作为最终版式依据。
3. 恢复 Docker Compose 工具后重建含 mysql2 的 Web 镜像，验证 `host.docker.internal` 或目标内网地址可达，并演练跨库更新后的镜像待同步恢复。

## 当前风险与待验证假设

- LibreOffice 与 Microsoft PowerPoint 可能存在字体/换行差异；部署镜像需要固定字体集和 LibreOffice 版本。
- Fast Preview 目前使用白色遮盖动态区域；在彩色背景、复杂排版中会产生偏差。真正去字背景尚未实现，不可代替真实预览。
- 外部企业指标库的表结构与写权限尚未给出；当前 Adapter 只对独立示例库的固定 `metric_record` 表开放参数化查询和修改。
- 外部源库与平台库无法共享事务；源库更新后平台镜像失败会报 `SYNC_PENDING`，但自动对账/重试流程尚未落地。
