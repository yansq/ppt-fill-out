...
- 当前阶段：`P5 审核与 FinalValue（进行中）`
- 项目状态：`P5_IN_PROGRESS`
- 最近更新：`2026-09-22`
- 下一里程碑：P5 浏览器交互回归；企业真实指标映射与容器交付验证
...
1. 检查 P5 审核面板在浏览器中的选值、手工值、退回和完成操作，并补足视觉/可用性问题。
2. 在企业目标内网取得真实指标库表结构、权限与连接参数，将示例固定映射扩展为受控映射；禁止原始 SQL 输入。
3. P6 开始前确认 FinalValue 与真实 PPTX 占位符的生成输入契约。按用户要求把 Docker Compose 镜像、数据源网络和跨库恢复的容器复验留到后期。
