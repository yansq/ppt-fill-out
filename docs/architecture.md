...
目标方案是解析时生成去除或隐藏动态文本后的静态页背景图，并返回动态 shape 的几何与样式。当前 P4 示例流程先使用已有模板 PNG，通过几何位置白色遮盖已绑定占位符并叠加 DOM 文本；彩色背景与复杂排版会有偏差，真正去字背景仍待实现。Fast Preview 只用于编辑反馈，P6 Real Preview 才是最终版式依据。
